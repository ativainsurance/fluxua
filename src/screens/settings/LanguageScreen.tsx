...
import { useSettings, SupportedLanguage } from '../../contexts/SettingsContext';
...
const LANGUAGES: Language[] = [
  ...
  { code: 'pt', name: 'Portuguese', nativeName: 'Português', flag: '🇧🇷', noteKey: 'language.brazilianPortuguese' },
  { code: 'es', name: 'Spanish', nativeName: 'Español', flag: '🇲🇽', noteKey: 'language.latinAmericanSpanish' },
];
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: spacing.base, paddingBottom: spacing.xxxl, gap: spacing.base }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ backgroundColor: colors.navy, borderRadius: radius.xl, padding: spacing.base, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', ...shadows.hero }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: spacing.md, paddingVertical: 6, borderRadius: radius.full, backgroundColor: colors.teal + '33', borderWidth: 1, borderColor: colors.teal + '55' }}>
            ...
            <Text style={{ fontSize: typography.xs, fontWeight: typography.semibold, color: colors.teal }}>{t('language.active')}</Text>
          </View>
        </View>

        {/* ── Language list ── */}
        <View style={{ gap: spacing.xs }}>
          <Text style={{ fontSize: typography.xs, fontWeight: typography.semibold, color: colors.textTertiary, letterSpacing: 0.8, textTransform: 'uppercase', paddingHorizontal: spacing.xs, marginBottom: 2 }}>{t('language.availableLanguages')}</Text>
            ...
            {LANGUAGES.map((lang, i) => {
              ...
              return (
                <React.Fragment key={lang.code}>
                  ...
                  <TouchableOpacity
                    style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing.base, paddingVertical: 14, gap: spacing.md, backgroundColor: isSelected ? colors.tealLight : colors.surface }}
                  ...
                  >
                    ...
                    <View style={{ flex: 1, gap: 3 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, flexWrap: 'wrap' }}>
                        ...
                        {lang.noteKey && (
                          <View style={{ borderRadius: radius.full, paddingHorizontal: 7, paddingVertical: 2, backgroundColor: colors.tealLight }}>
                            <Text style={{ fontSize: 10, fontWeight: typography.semibold, letterSpacing: 0.2, color: colors.teal }}>{t(lang.noteKey)}</Text>
                          </View>
                        )}
                      </View>
        ...
        <View style={{ flexDirection: 'row', gap: spacing.md, backgroundColor: colors.surface, borderRadius: radius.xl, padding: spacing.base, ...shadows.sm, borderLeftWidth: 3, borderLeftColor: colors.teal }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: typography.sm, fontWeight: typography.bold, color: colors.textPrimary, marginBottom: 4 }}>{t('language.multilingualTitle')}</Text>
            <Text style={{ fontSize: typography.xs, color: colors.textSecondary, lineHeight: 18 }}>
              {t('language.multilingualDescription')}
            </Text>
          </View>
        </View>
